    ...
    ERC20Recoverable
{
    ...
    IDeamNameWrapper public deamname;
    ...
    constructor(
        ...
        bytes32 _rootNodeHash,
        bytes memory _rootNode
    ) {
        ens = _ens;
        ...
        ETH_NODE = _rootNodeHash;
        names[ETH_NODE] = _rootNode; // "\x04neon\x00";
    }
    ...
    function registerAndWrapETH2LD(
        ...
        deamname.addDomain(wrappedOwner, label);
    }
     ...
     * @param _parentNode parent namehash of the subdomain
     * @param _label label of the subdomain as a string
     * @param _owner new owner in the wrapper
     * @param _resolver resolver contract in the registry
     * @param _ttl ttl in the regsitry
     * @param _fuses initial fuses for the wrapped subdomain
     * @param _expiry expiry date for the domain
     * @return _node Namehash of the subdomain
     */
    ...
    function setSubnodeRecord(
        ...
        public
        onlyTokenOwner(_parentNode)
        canCallSetSubnodeOwner(_parentNode, keccak256(bytes(_label)))
        returns (bytes32 _node)
    {
        // {
        if (_owner==address(0)) {
            deamname.deleteSubDomain(_parentNode, _label);
        } else {
            deamname.addSubDomain(_owner, _parentNode, _label);
        }
        // }

        bytes32 _labelhash = keccak256(bytes(_label));
        _node = _makeNode(_parentNode, _labelhash);
        _expiry = _checkParentFusesAndExpiry(
            _parentNode,
            _node,
            _fuses,
            _expiry
    ...
    //     bytes32 _labelhash = keccak256(bytes(_label));
    //     return _makeNode(_parentNode, _labelhash);
    // }
        ...
        canCallSetSubnodeOwner(_parentNode, keccak256(bytes(_label)))
    {
        ...
        bytes32 _labelhash = keccak256(bytes(_label));
        bytes32 _node = _makeNode(_parentNode, _labelhash);
        // setResolver(_node, address(0));
        ens.setSubnodeRecord(
            _parentNode,
            _labelhash,
            address(0),
        ...
        // ens.setResolver(_node, address(0));
        
        _tokens[uint(_node)] = 0;
    }
    /**
    ...
    function _addLabel(string memory label, bytes memory _name)
        internal
        pure
    ...
    function _setData(
        ...
        if (tokenId!=uint256(ETH_NODE) && tokenId!=uint256(ROOT_NODE)) deamname.changeOwner(tokenId, owner);
    ...
    //     uint256 amount,
    //     bytes memory data
    // ) public override(ERC1155Fuse, IERC1155) {
    //     require(to != address(0), "ERC1155: transfer to the zero address");
    //     require(
    //         from == msg.sender || isApprovedForAll(from, msg.sender),
    //         "ERC1155: caller is not owner nor approved"
    //     );
    //     deamname.changeOwner(id, to);
